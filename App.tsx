...
function resolveUrl(urlStr: string): string {
  ...
  // Helper to validate if an origin is likely our backend container
  const isValidOrigin = (orig: string): boolean => {
    if (!orig || orig === 'null' || !orig.startsWith('http')) return false;
    const lower = orig.toLowerCase();
    // Exclude standard sandbox/iframe container origins that don't host our backend
    if (lower.includes('google') || lower.includes('ai.studio') || lower.includes('gstatic') || lower.includes('webcontainer')) {
      return false;
    }
    return true;
  };

  // 1. HIGHLY PREFERRED: inspect import.meta.url (the module itself)
  try {
    const metaUrl = import.meta.url;
    if (metaUrl && metaUrl.startsWith('http')) {
      const parsed = new URL(metaUrl);
      if (isValidOrigin(parsed.origin)) {
        return `${parsed.origin}${cleanUrl}`;
      }
    }
  } catch (e) {}

  // 2. Scan script tags in the DOM for our app's assets
  if (typeof document !== 'undefined') {
    ...
    // 3. Scan link tags in the DOM (like stylesheets or main css)
    try {
  ...
  // 4. Try document.baseURI
  try {
    if (typeof document !== 'undefined' && document.baseURI && isValidOrigin(document.baseURI)) {
      const parsed = new URL(document.baseURI);
      return `${parsed.origin}${cleanUrl}`;
    }
  } catch (e) {}

  // 5. Try window.location.origin / window.location.href
  try {
    if (typeof window !== 'undefined' && window.location) {
      if (isValidOrigin(window.location.origin)) {
        return `${window.location.origin}${cleanUrl}`;
      }
      if (isValidOrigin(window.location.href)) {
  ...
  // 6. Hardcoded Dev & Pre dynamically-inspected URL fallbacks based on metadata as absolute fallback
  try {
    const currentHost = typeof window !== 'undefined' && window.location ? window.location.host : '';
    if (currentHost.includes('pre-p7x4yaetbkifs55d7tf6la')) {
      return `https://ais-pre-p7x4yaetbkifs55d7tf6la-167565513046.asia-southeast1.run.app${cleanUrl}`;
    }
  } catch (e) {}
  
  const devFallback = 'https://ais-dev-p7x4yaetbkifs55d7tf6la-167565513046.asia-southeast1.run.app';
  return `${devFallback}${cleanUrl}`;
}
...
const ChartIndicators = ({ data, indicators }: { data: any[], indicators: any }) => {
  ...
  useEffect(() => {
    ...
    let upTrend: any = null;
    let downTrend: any = null;
    if (indicators.supertrend) {
       upTrend = chart.addSeries(LineSeries, { color: '#10b981', lineWidth: 3, title: 'ST UP' });
       downTrend = chart.addSeries(LineSeries, { color: '#ef4444', lineWidth: 3, title: 'ST DOWN' });
    ...
    // Custom Interactive Tooltip setup
    chart.subscribeCrosshairMove((param) => {
      ...
      if (candleSeries) {
        ...
        if (cData && typeof cData === 'object') {
          const closeVal = 'close' in cData ? (cData as any).close : null;
          if (closeVal !== undefined && closeVal !== null) {
            candleText = `
              <div class="flex items-center justify-between gap-4 py-0.5 border-b border-[#1A1A1A]/5">
                <span class="font-sans font-black uppercase text-[8px] opacity-50">Price</span>
                <span class="font-mono font-bold text-[#1A1A1A]">$${closeVal.toFixed(2)}</span>
              </div>
            `;
          }
        }
      ...
      if (indicators.ema && emaSeries) {
        ...
        if (emaData && typeof emaData === 'object' && 'value' in emaData) {
          const val = (emaData as any).value;
          if (val !== undefined && val !== null) {
            emaText = `
              <div class="flex items-center justify-between gap-4 py-0.5 border-b border-[#1A1A1A]/5">
                <span class="flex items-center gap-1 font-sans font-black uppercase text-[8px] text-blue-600">
                  <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> EMA 50
                </span>
                <span class="font-mono font-bold text-[#1A1A1A]">$${val.toFixed(2)}</span>
              </div>
            `;
          }
        }
      ...
      if (indicators.supertrend) {
        ...
        if (upTrend) {
          const utData = param.seriesData.get(upTrend);
          if (utData && typeof utData === 'object' && utData !== null && 'value' in utData) {
            stVal = (utData as any).value;
            isSTUp = true;
          }
        }
        
        if ((stVal === undefined || stVal === null) && downTrend) {
          const dtData = param.seriesData.get(downTrend);
          if (dtData && typeof dtData === 'object' && dtData !== null && 'value' in dtData) {
            stVal = (dtData as any).value;
            isSTUp = false;
          }
        }
        ...
        if (stVal !== undefined && stVal !== null) {
          stText = `
            <div class="flex items-center justify-between gap-4 py-0.5 border-b border-[#1A1A1A]/5">
              <span class="flex items-center gap-1 font-sans font-black uppercase text-[8px] ${isSTUp ? 'text-emerald-600' : 'text-[#FF4400]'}">
                <span class="w-1.5 h-1.5 rounded-full ${isSTUp ? 'bg-emerald-500' : 'bg-[#FF4400]'}"></span> ST ${isSTUp ? 'UP' : 'DOWN'}
              </span>
              <span class="font-mono font-bold ${isSTUp ? 'text-emerald-700' : 'text-[#FF4400]'}">$${stVal.toFixed(2)}</span>
            </div>
          `;
        }
      }
      ...
      try {
        ...
        if (typeof timeVal === 'number') {
          const dateObj = new Date(timeVal * (timeVal < 10000000000 ? 1000 : 1));
          formattedTime = format(dateObj, 'yyyy-MM-dd HH:mm');
        } else if (typeof timeVal === 'object' && timeVal !== null && 'year' in timeVal) {
          const tObj = timeVal as any;
          formattedTime = `${tObj.year}-${String(tObj.month).padStart(2, '0')}-${String(tObj.day).padStart(2, '0')}`;
        } else {
          formattedTime = String(timeVal);
        }
        timeText = `<div class="text-[8px] text-[#1A1A1A]/40 font-sans font-bold border-b border-[#1A1A1A]/10 pb-1 mb-1 uppercase">${formattedTime}</div>`;
      } catch (e) {
        // ignore
      }
  ...
  return (
    <div className="flex flex-col h-full relative">
      <div ref={chartContainerRef} className="flex-1 cursor-crosshair" />
      <div 
        ref={tooltipRef}
        className="absolute hidden z-30 pointer-events-none p-2.5 border-2 border-[#1A1A1A] bg-white font-sans shadow-sm rounded-sm"
        style={{ left: 0, top: 0, minWidth: '140px' }}
      />
